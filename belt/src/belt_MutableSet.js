...
function split(d, key) {
  ...
  let cmp = d.cmp;
  let i = Belt_SortArray.binarySearchByU(arr, key, cmp);
  let len = arr.length;
  if (i >= 0) {
    return [
      [
...
function intersect(a, b) {
  ...
  let k = Belt_SortArray.intersectU(tmp, 0, sizea, tmp, sizea, sizeb, tmp2, 0, cmp);
  return {
    cmp: cmp,
...
function diff(a, b) {
  ...
  let k = Belt_SortArray.diffU(tmp, 0, sizea, tmp, sizea, sizeb, tmp2, 0, cmp);
  return {
    cmp: cmp,
    data: Belt_internalAVLset.fromSortedArrayAux(tmp2, 0, k)
  };
...
function union(a, b) {
  ...
  let k = Belt_SortArray.unionU(tmp, 0, sizea, tmp, sizea, sizeb, tmp2, 0, cmp);
  return {
    cmp: cmp,
    data: Belt_internalAVLset.fromSortedArrayAux(tmp2, 0, k)
  };
